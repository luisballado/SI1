...
%DIF ADD protocolo8859.tex      Thu Aug 24 12:13:35 2023
...
En \'{u}ltimas \DIFdelbegin \DIFdel{decadas }\DIFdelend \DIFaddbegin \DIFadd{d�cadas }\DIFaddend se han propuesto diversas soluciones globales de planificaci\'{o}n basadas en distintas t\'{e}cnicas de programaci\'{o}n (Mixed Integer Linear Programming (MILP), Nonlinear programming (NP) y Dynamic Programming (DP)), pero su \DIFdelbegin \DIFdel{escala }\DIFdelend \DIFaddbegin \DIFadd{complejidad }\DIFaddend computacional crece exponencialmente conforme aumenta el espacio de b\'{u}squeda.\\
...
Tambi�n se han propuesto diversas t\'{e}cnicas de Inteligencia Computacional para el problema de planificaci\'{o}n de trayectorias (\DIFdelbegin \DIFdel{Algoritmos Gen\'{e}ticos GA, Ant Colony Optimization }\DIFdelend \DIFaddbegin \DIFadd{algoritmos gen\'{e}ticos (GA), algoritmo de optimizaci�n por colonia de hormigas }\DIFaddend (ACO) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ZHAO2020} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{ZHAO2020} }\hskip0pt%DIFAUXCMD
}\DIFaddend [\citenum{ZHAO2020}], \DIFdelbegin \DIFdel{Particle Swarm Optimization }\DIFdelend \DIFaddbegin \DIFadd{optimizaci�n por enjambre de particulas }\DIFaddend (PSO) y \DIFdelbegin \DIFdel{Evolucion Diferencial }\DIFdelend \DIFaddbegin \DIFadd{evoluci�n diferencial }\DIFaddend (DE)). Estos algoritmos han demostrado crear rutas navegables para los VANT y son \DIFdelbegin \DIFdel{apliamente }\DIFdelend \DIFaddbegin \DIFadd{ampliamente }\DIFaddend usados para problemas de \DIFdelbegin \DIFdel{planificacion }\DIFdelend \DIFaddbegin \DIFadd{planificaci�n }\DIFaddend de rutas complejos. Desafortunadamente, estas t�cnicas requieren de amplios recursos computacionales, lo que las hace inadecuadas para implementar en los VANTS actuales.
...
\DIFdelbegin \DIFdel{Desarrollar una arquitectura descentralizada de control, implementada en cada uno de los miembros de un conjunto de $\mathcal{V}$ veh\'{i}culos a\'{e}reos no tripulados, que refleje una estrategia de exploraci\'{o}n multi-VANT para la exploraci�n de un medio ambiente dado.El prop�sito de esta arquitectura de control es que cada uno de los VANT participe de manera independiente pero coordinada para reducir el tiempo total }\DIFdelend \DIFaddbegin \DIFadd{Dada un �rea de inter�s desconocida que se desea explorar denotada como $\mathcal{A} \subset \mathbb{R}^{3}$, que puede tomar valores libres o ocupados de notados como $\mathcal{A}_{libre}$, $\mathcal{A}_{obs}$.}\\

\DIFadd{Un voxel $v$ que representa el espacio contenido en $\mathcal{A}$ que es obtenido dividiendo recursivamente el �rea de inter�s $\mathcal{A}$ en ocho partes de igual tama�o. La regi�n ocupada son obtenidas mediante un sensor basado en un modelo de ocupaci�n probabil�stico.}\\
...
\DIFadd{Determinar el conjunto de tareas para cada VANT, as� como el conjunto de rutas que maximize la cobertura minimizando el tiempo }\DIFaddend de exploraci�n \DIFdelbegin \DIFdel{, colaborando y compartiendo informaci�n eficazmente con los dem�s miembros del equipo.}\DIFdelend \DIFaddbegin \DIFadd{y la energ�a consumida.}\\
\DIFaddend 
...
\item%DIFAUXCMD
\DIFdelend Asegurar la consistencia de la informaci�n recolectada y fusionada en un �nico mapa, compartido entre todos los robots
\end{itemize}

Con base en lo anterior, surgen las siguientes preguntas de investigaci\'{o}n:
\begin{itemize}
  \item \DIFdelbegin \DIFdel{\textquestiondown Qu\'{e} acciones deber\'{a}n de realizar los VANTS para explorar el espacio completo lo m\'{a}s r\'{a}pido posible}\DIFdelend \DIFaddbegin \DIFadd{�Qu� estrategias pueden emplearse para optimizar la asignaci�n de tareas y coordinaci�n entre m�ltiples VANTS para el problema de exploraci�n de un ambiente desconocido}\DIFaddend ?
...
\item \DIFadd{�}\DIFaddend C�mo asegurar que la \DIFdelbegin \DIFdel{nueva }\DIFdelend informaci�n que aporte cada uno de los VANTs se integre correctamente a la informaci�n ya conocida \DIFdelbegin \DIFdel{y se distribuya entre todos los miembros del equipo}\DIFdelend \DIFaddbegin \DIFadd{para generar mapas precisos y completos del entorno explorado}\DIFaddend ?
\end{itemize}
...
\item \textbf{\DIFdelbegin \DIFdel{Comparaci\'{o}n y an\'{a}lisis}\DIFdelend \DIFaddbegin \DIFadd{Construcci�n de mapas colaborativo}\DIFaddend }. \DIFdelbegin \DIFdel{Comparar y analizar los resultados obtenidos con enfoques existentes en la coordinaci\'{o}n multi-VANT, mostrando las ventajas y desventajas de la estrategia propuesta, al compararla con otras soluciones propuestas en la literatura.
...
Matem\'{a}ticamente, el problema de planificaci�n de rutas es resuelto a trav�s del modelado del medio ambiente utilizando grafos, siendo un grafo una representaci\'{o}n matem\'{a}tica de v\'{e}rtices y aristas. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{4082128}[\citenum{4082128}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{4082128}[\citenum{4082128}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , al mejorar el algoritmo de Dijkstra para el robot Shakey, logr\'{o} navegar en una habitaci\'{o}n que conten\'{i}a obst\'{a}culos fijos. El objetivo principal del algoritmo A* es la eficiencia en la planificaci\'{o}n de rutas. El algoritmo D*, propuesto por \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{351061}[\citenum{351061}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{351061}[\citenum{351061}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , ha demostrado operar de manera eficiente ante obst\'{a}culos din\'{a}micos; en comparaci\'{o}n con el algoritmo A* que vuelve a ejecutarse al encontrarse con un obst\'{a}culo no previsto inicialmente, el algoritmo D* usa la informaci\'{o}n previa para buscar una nueva ruta hacia el objetivo. 

El algoritmo RRT, propuesto por \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
}\DIFaddend , es ampliamente usado para la planificaci\'{o}n de rutas en robots modernos. El algoritmo construye de forma incremental una estructura de \'{a}rbol mediante un muestreo aleatorio en el espacio de configuraciones, uniendo aleatoriamente nuevas posiciones al \'{a}rbol existente hasta alcanzar la posici�n final. Las modificaciones realizadas al algoritmo RRT por \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Karaman2011}[\citenum{Karaman2011}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{Karaman2011}[\citenum{Karaman2011}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , incorporando una heur\'{i}stica de costo por recorrer, permite encontrar rutas casi \'{o}ptimas de manera eficiente. Este enfoque es ampliamente usado en problemas de navegaci\'{o}n aut\'{o}noma y planificaci\'{o}n de movimiento.\\

Los recientes trabajos de \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yang2022far}[\citenum{yang2022far}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{yang2022far}[\citenum{yang2022far}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , siguen demostrando la capacidad de implementaci\'{o}n de algoritmos cl�sicos de planificaci�n de trayectorias, como los grafos de visibilidad, para tareas en entornos conocidos y no conocidos, utilizando una representaci\'{o}n del ambiente a base de \DIFdelbegin \DIFdel{poligonos}\DIFdelend \DIFaddbegin \DIFadd{pol�gonos}\DIFaddend , logrando un r\'{a}pido planificador que tambi\'{e}n resuelve los obst\'{a}culos nuevos en el ambiente, logrando resultados comparables a las \DIFdelbegin \DIFdel{estategias }\DIFdelend \DIFaddbegin \DIFadd{estrategias }\DIFaddend m�s recientes como A*,D* e inclusive RRT*.\\
...
Adem\'{a}s de la planificaci\'{o}n de rutas, la coordinaci\'{o}n de m\'{u}ltiples robots para la exploraci�n requiere de una estrategia \DIFdelbegin \DIFdel{una }\DIFdelend \DIFaddbegin \DIFadd{de }\DIFaddend comunicaci\'{o}n efectiva, que garantice la coherencia del mapa que se va generando. Se han investigado diferentes protocolos de comunicaci\'{o}n y estrategias de intercambio de informaci\'{o}n para permitir la colaboraci\'{o}n. Algunos enfoques utilizan comunicaci\'{o}n directa entre los robots, mientras que otros emplean una arquitectura de red donde los m\'{u}ltiples robots se comunican a trav\'{e}s de una infraestructura descentralizada \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{10120943}[\citenum{10120943}] }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{10120943}[\citenum{10120943}] }\hskip0pt%DIFAUXCMD
}\DIFaddend mostrando la tolerancia a fallas en equipos para tareas de b\'{u}squeda y rescate.\\\\ %La elecci\'{o}n del enfoque depende de las caracter\'{i}sticas de la aplicaci\'{o}n y las restricciones del sistema.\\
...
En el Centro de Investigaci\'{o}n y Estudios Avanzados del \DIFdelbegin \DIFdel{Institudo }\DIFdelend \DIFaddbegin \DIFadd{Instituto }\DIFaddend Polit\'{e}cnico Nacional Unidad Tamaulipas se han realizado investigaciones en el \'{a}rea de exploraci\'{o}n multi-robot y dise\~{n}o de prototipos de VANTS, lo cual sirve como antecedente para este trabajo. Este relevante desarrollo, propuesto por \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
}\DIFaddend , tiene como objetivo principal el despliegue de una estrategia de coordinaci\'{o}n para m\'{u}ltiples robots m\'{o}viles basado en un enfoque de auto-ofertas. Tambi�n se han desarrollado trabajos de investigaci�n \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CINVES2013}[\citenum{CINVES2013}] }\hskip0pt%DIFAUXCMD
cuyos objetivos son }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{CINVES2013}[\citenum{CINVES2013}] }\hskip0pt%DIFAUXCMD
cuyo objetivo es }\DIFaddend la generaci\'{o}n de mapas fotogr\'{a}ficos utilizando veh\'{i}culos a\'{e}reos no tripulados de baja altitud. \\
...
Un resultado interesante es el propuesto por \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CINVES2015}[\citenum{CINVES2015}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{CINVES2015}[\citenum{CINVES2015}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , del Cinvestav Unidad Guadalajara, que se centra en las posibilidades de navegaci\'{o}n aut\'{o}noma de un VANT. Tambi�n, el trabajo propuesto por \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{CINVES2021}[\citenum{CINVES2021}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{CINVES2021}[\citenum{CINVES2021}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , tiene como objetivo el desarrollo de \DIFaddbegin \DIFadd{una }\DIFaddend arquitectura para un �nico VANT en tareas de exploraci\'{o}n. Ninguno de estos trabajos \DIFdelbegin \DIFdel{aborada }\DIFdelend \DIFaddbegin \DIFadd{aborda }\DIFaddend la exploraci�n multi-VANT.\\
...
La exploraci\'{o}n de un ambiente desconocido empleando \DIFdelbegin \DIFdel{m\'{u}ltiples veh\'{i}culos a\'{e}reos no tripulados }\DIFdelend \DIFaddbegin \DIFadd{multi-VANT }\DIFaddend es un \'{a}rea relativamente nueva y con mucho crecimiento en los \'{u}ltimos a\~{n}os. Se han abordado una variedad de temas distintos para lograr la exploraci\'{o}n aut\'{o}noma, desde la planificaci\'{o}n de rutas para m\'{u}ltiples robots en tareas de exploraci\'{o}n \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sharma2016}[\citenum{Sharma2016}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{Sharma2016}[\citenum{Sharma2016}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , estrategias para la coordinaci\'{o}n y protocolos de comunicaci\'{o}n. Diversos estudios multi-VANT se han realizado para tareas como el monitoreo ambiental \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{SurveyCollab2019}[\citenum{SurveyCollab2019}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{SurveyCollab2019}[\citenum{SurveyCollab2019}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , agricultura de \DIFdelbegin \DIFdel{presici\'{o}n \mbox{%DIFAUXCMD
\cite{10011226}[\citenum{10011226}] }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{precisi\'{o}n \mbox{%DIFAUXCMD
\citeauthor{10011226}[\citenum{10011226}] }\hskip0pt%DIFAUXCMD
}\DIFaddend y operaciones de b\'{u}squeda y rescate \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{SHAKHATREH2019}[\citenum{SHAKHATREH2019}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{SHAKHATREH2019}[\citenum{SHAKHATREH2019}]}\hskip0pt%DIFAUXCMD
}\DIFaddend .\\
...
Las bases para la exploraci\'{o}n aut\'{o}noma e \DIFdelbegin \DIFdel{inovaciones }\DIFdelend \DIFaddbegin \DIFadd{innovaciones }\DIFaddend son heredadas de algoritmos ya empleados en la rob\'{o}tica m\'{o}vil \DIFdelbegin \DIFdel{. }%DIFDELCMD < [%%%
\DIFdel{ver cuadro \ref{tab:pathsummary}}%DIFDELCMD < ]%%%
\DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(tabla \ref{pathsummary}). }\DIFaddend Uno de los primeros trabajos en la exploraci\'{o}n con robots, es la propuesta de \DIFdelbegin \DIFdel{fonteras \mbox{%DIFAUXCMD
\cite{YAMAUCHI1997}[\citenum{YAMAUCHI1997}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{fronteras por \mbox{%DIFAUXCMD
\citeauthor{YAMAUCHI1997}[\citenum{YAMAUCHI1997}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , donde define como una frontera a la l\'{i}nea entre las zonas exploradas y las no exploradas de un \'{a}rea de inter\'{e}s. Durante la navegaci\'{o}n la informaci\'{o}n percibida por el robot crece, moviendo las fronteras hasta que no existan m\'{a}s fronteras. En el trabajo de \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Faria2019}[\citenum{Faria2019}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{Faria2019}[\citenum{Faria2019}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , se combina la estrategia basada en fronteras con t\'{e}cnicas de planificaci\'{o}n de trayectorias Lazy Theta* en un VANT.\\  
...
En lo que respecta a la organizaci�n de la arquitectura de control, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\citeauthor{BROOKS1986}[\citenum{BROOKS1986}]}\hskip0pt%DIFAUXCMD
  ...
  Un enfoque muy recurrente para abordar problemas complejos, como la planificaci�n de trayectorias, la fusi�n de datos y la distribuci�n de tareas, es el uso de las metaheur\'{i}sticas \DIFdelbegin \DIFdel{Bio-inspirada}\DIFdelend \DIFaddbegin \DIFadd{bio-inspiradas}\DIFaddend . Estas estrategias se inspiran en sistemas y procesos biol\'{o}gicos para resolver problemas complejos de optimizaci\'{o}n. Existen varios tipos de metaheur\'{i}sticas bio-inspiradas:
...
Los primeros trabajos multi-VANT se encuentran en las aportaciones de \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{SHEN2011}[\citenum{SHEN2011}]}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{SHEN2011}[\citenum{SHEN2011}]}\hskip0pt%DIFAUXCMD
}\DIFaddend , que hacen uso de un VANT con la propuesta de dos planificadores de trayectorias con un control proporcional con retroalimentaci\'{o}n y basados en RRT*, para conseguir una representaci\'{o}n del mundo en 2D empleando un sensor tipo LiDAR. Por otra parte, los trabajos de \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{GRZONKA2012}[\citenum{GRZONKA2012}] }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeauthor{GRZONKA2012}[\citenum{GRZONKA2012}] }\hskip0pt%DIFAUXCMD
}\DIFaddend tambi\'{e}n hacen una representaci\'{o}n del entorno en 2D, haciendo uso de algoritmos que trabajan en mapas densos tipo grid, y del algoritmo D* lite para la planificaci\'{o}n de trayectorias. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\cite{USENKO2017}[\citenum{USENKO2017}] proponen el uso del mapa centrando al robot en un \DIFdelbegin \DIFdel{circulo }\DIFdelend \DIFaddbegin \DIFadd{c�rculo }\DIFaddend tridimensional de tama\~{n}o fijo, plantea el problema de replanificaci\'{o}n local como la optimizaci\'{o}n de una funci\'{o}n de costo con un t\'{e}rmino que penaliza las desviaciones de posici\'{o}n y velocidad de la trayectoria de referencia. La trayectoria de referencia es construida como curva de Bezier uniforme, simplificando el c\'{a}lculo. \\
%Hacen uso de un paquete de optimizaci\'{o}n no lineal\\

\cite{MOHTA2017}[\citenum{MOHTA2017}] hacen uso de un mapa h\'{i}brido formado con la combinaci\'{o}n de un mapa 3D con un mapa global en 2D, usan un planificador A* en un grafo h\'{i}brido con la informaci\'{o}n 3D y 2D, formulan un problema de programaci\'{o}n \DIFdelbegin \DIFdel{cu\'{a}dratica }\DIFdelend \DIFaddbegin \DIFadd{cuadr�tica }\DIFaddend para la generaci\'{o}n de trayectorias agregando un t\'{e}rmino en la funci\'{o}n de costo sobre el error entre la trayectoria y los segmentos de l\'{i}nea del camino. La trayectoria se representa como un polinomio de s\'{e}ptimo orden, donde la asignaci\'{o}n de tiempo a cada segmento utiliza un perfil de velocidad trapezoidal. \\
