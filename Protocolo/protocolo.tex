...
\begin{abstract}
 ...
 %\st{La importancia de la rob\'{o}tica de servicios en la actualidad es innegable. Estos avances est\'{a}n revolucionando la forma en que interactuamos con el mundo, ofreciendo un amplio abanico de aplicaciones en diversos sectores. Desde veh\'{i}culos aut\'{o}nomos, robots m\'{o}viles en l\'{o}gistica, hasta la exploraci\'{o}n espacial, la rob\'{o}tica de servicios ha demostrado ser \'{u}til en entornos donde los seres humanos pueden enfrentar riesgos o dificultades.}\\
  ...
  La exploraci\'{o}n multi-robot ha surgido como un enfoque prometedor para el mapeo eficiente de entornos desconocidos. Un enfoque colaborativo ofrece una mayor eficiencia de exploraci\'{o}n, una obtenci\'{o}n de informaci\'{o}n m\'{a}s r\'{a}pida y amplias capacidades de cobertura en comparaci\'{o}n con implementaci\'{o}nes donde se se emplea un solo robot. Sin embargo, la exploraci\'{o}n multi-robot plantea diversos desaf\'{i}os que deben abordarse para su implementaci\'{o}n como la localizaci\'{o}n, mapeo y navegaci\'{o}n aut\'{o}noma.\\
  En la \'{u}ltima decada, se ha tenido un aumento en la investigaci\'{o}n y el desarrollo en el campo de los m\'{u}ltiples V\'{e}hiculos A\'{e}reos No Tripulados (VANT) lo que ha dado lugar a importantes avances e innovaciones. Los sistemas multi-VANT permiten la adquisici\'{o}n simult\'{a}nea de datos desde m\'{u}ltiples puntos de vista, lo que permite mejorar el mapeo de entornos desconocidos. El uso de algoritmos de coordinaci\'{o}n inteligente, la toma de decisiones descentralizada mejora la eficiencia de estos sistemas. Adem\'{a}s, los avances en los protocolos de comunicaci\'{o}n permiten una colaboraci\'{o}n fluida, lo que mejora su capacidad para navegar, explorar y adquirir datos de \'{a}reas grandes y complejas. Adem\'{a}s, la integraci\'{o}n de sensores de \'{u}ltima generaci\'{o}n mejora la precisi\'{o}n y confiabilidad de los sistemas multi-VANT en varios dominios, incluida la gesti\'{o}n de desastres, la agricultura de precisi\'{o}n, la inspecci\'{o}n de infraestructura y la vigilancia militar \cite{SHAKHATREH2019} o en espectaculares animaciones a\'{e}reas. \cite{DRONESHOW2023}
  Dichas aplicaciones suelen carecer de autonom\'{i}a. Para que un robot se considere aut\'{o}nomo deber\'{a} tomar decisiones y realizar tareas sin necesidad de que alguien le diga qu\'{e} hacer o guiarlo paso a paso. Tener la capacidad de percibir su entorno y usar la informaci\'{o}n para decidir c\'{o}mo moverse son considerados altos niveles de autonom\'{i}a. Para llegar a ello el robor debe resolver primero problemas como su localizaci\'{o}n, construir el mapa de su entorno para posteriormente usarlo para navegar dentro de \'{e}l.
...
Los controles de bajo nivel responden muy bien a t\'{e}cnicas de teoria de control \cite{ramirez2001}. Considerando como entrada del sistema la posici\'{o}n que deseamos y su orientaci\'{o}n. El error ser\'{a} la diferencia entre la posici\'{o}n deseada y la posici\'{o}n actual\\
  ...
  While metaheuristics have proven to be effective in solving a wide range of optimization problems, their adoption in the robotics field has been more limited for several reasons:

  \begin{itemize}
    \item Complexity and Real-Time Constraints: Robotics often involves real-time decision-making, where robots must respond quickly to changing environments. Metaheuristics typically require extensive computational resources and iterations to converge to an optimal solution, which may not be feasible in real-time robotics applications. Real-time control and planning in robotics often require algorithms with low computational complexity, such as classical planning or reactive control approaches.

    \item Deterministic Solutions: In many robotics applications, especially those involving safety-critical tasks or precise control, deterministic and predictable solutions are preferred over stochastic solutions offered by metaheuristics. Metaheuristics provide approximate solutions with varying degrees of optimality, which may not be suitable for tasks requiring precise control or safety guarantees.

    \item Model-Based Optimization: Many robotics problems can be effectively solved using model-based optimization techniques. With known dynamics models and environmental constraints, model-based methods such as optimal control or trajectory optimization can provide analytical or numerical solutions with guaranteed performance. These approaches can exploit the problem structure and specific constraints, leading to more efficient and reliable solutions compared to general-purpose metaheuristics.

    \item Task-Specific Algorithms: Robotics often involves specific tasks and domains that have been extensively studied, resulting in task-specific algorithms tailored to those domains. These tailored approaches are often more efficient and effective for solving the specific problems addressed in robotics, making general-purpose metaheuristics less attractive.

    \item Hardware and Energy Limitations: Robotics systems typically have limited hardware resources and are often constrained by energy consumption. Metaheuristics, which often require large populations or extensive iterations for convergence, may not be well-suited for resource-constrained robotic platforms.
  \end{itemize}
  
  However, it's important to note that there are certainly areas within robotics where metaheuristics have been successfully applied, such as robot path planning in complex environments, swarm robotics, or task allocation in multi-robot systems. Additionally, hybrid approaches that combine metaheuristics with model-based optimization or task-specific algorithms can leverage the strengths of both and provide effective solutions for robotics applications.
...
\cite{10120941} clasifican los Veh\'{i}culos A\'{e}reos No Tripulados (VANT) principalmente como: \textbf{Ala Fija, Ala Rotatoria y Hibrido}\\
\textbf{Ala fija} - cuentan con una ala rigida, similar a los aviones comerciales que conocemos. Requieren un area reservada para el despegue\\
\textbf{Ala rotatoria} - \\
\textbf{Hibrido} -  \\
...
\caption[mnodels pathsummary]{M\'{e}todos para planificaci\'{o}n de trayectorias}\label{tab:pathsummary}
\end{table}%
...
\hline
\textbf{E1.A1.} Revisi\'{o}n estado del arte\footnote{Revisi\'{o}n de alertas de trabajos relacionados}
...
\hline
\textbf{E1.A4.} Elaboraci\'{o}n de soluci\'{o}n \off[1] \on[3] \off[8] \\
\hline
\textbf{E1.A5.} Documentaci\'{o}n Etapa 1 \on[4]  \off[8] \\
\hline
\textbf{E1.A6.} Revisi\'{o}n de tesis Etapa 1 \off[3] \on[1]  \off[8] \\
\hline
...
\textbf{E2.A3.} Desarrollo de algoritmo de exploraci\'{o}n
\off[3] \on[2] \off[7] \\
\hline
\textbf{E2.A4.} Implementaci\'{o}n un solo VANT\footnote{Se considera un solo agente que resuelva la tarea de exploraci\'{o}n aut\'{o}noma con evaci\'{o}n de obst\'{a}culos}
\off[3] \on[2] \off[7] \\
\hline
\textbf{E2.A6.} Simulaci\'{o}n un solo VANT
\off[4] \on[1] \off[7] \\
\hline
\textbf{E2.A7.} Desarrollo de coordinaci\'{o}n
\off[4] \on[3] \off[5] \\
\hline
\textbf{E2.A8.} Implementaci\'{o}n multi-VANT
\off[6] \on[2] \off[4] \\
\hline
\textbf{E2.A9.} Simulaci\'{o}n multi-VANT
\off[6] \on[1] \off[5] \\
\hline
\textbf{E2.A10.} Documentaci\'{o}n Etapa 2
\off[4] \on[4] \off[4] \\
\hline
\textbf{E2.A11.} Revisi\'{o}n de tesis Etapa 2
\off[7] \on[1] \off[4] \\
\hline
...
\noindent\hrulefill IDEAS \noindent\hrulefill\\
  ...
  \textbf{COSAS DE ESTADO DEL ARTE}\\
  
  Estos problemas ya se han resulto en varios robots terrestres llegando a tener soluciones distribuidas o resultos los problemas de colisi\'{o}n, navegaci\'{o}n, mapeo y se han propuestos buenos algoritmos que formar\'{a}n parte de la arquitectura de software para resolver.\\
